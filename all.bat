...
set "in=input.mp4"
REM tady menit nazev vstupniho videa

REM 3x3 fragmenty pojmenovane 1..9 (radky 0..2, sloupce 0..2)
for /L %%r in (0,1,2) do (
  for /L %%c in (0,1,2) do (
    set /A str=%%r*3+%%c+1
    ffmpeg -i "%in%" -vf "crop=iw/3:ih/3:iw/3*%%c:ih/3*%%r" "!str!.mp4" -y
  )
)
...
REM ffmpeg -i input.mp4 -filter:v "crop=iw*2/2.4:ih:iw/3:0" dve_tretiny_vpravo-B.mp4 -y
ffmpeg -i input.mp4 -filter:v "crop=iw*2/3:ih:0:0" dve_tretiny_vlevo.mp4 -y

REM ----1/3---- horizontalne
